...
| `ADD . /deps/my-agent` and the install loop | `dependencies` | Each local path entry becomes an `ADD` into `/deps/<name>` and is pip-installed. PyPI names become a `pip install` line instead. `ADD .` copies the **whole directory**, so a `.dockerignore` next to `langgraph.json` is mandatory; without one the sample image contained `.venv` (196MB), `.env` and `.langgraph_api/` (inspected in the built image). `langgraph dockerfile --add-docker-compose` writes one; the plain command does not. See [module 07](./07-standalone-helm-deploy.md#practical-build-rules). |
