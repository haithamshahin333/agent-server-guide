...
are identical for both; modules 07 and 08 branch.

## How to read this guide

| If you are… | Read | You can skim |
| --- | --- | --- |
| A developer building or porting an agent | 00, 01, 02 or 03, 04, 05 | 06 (the "What to focus on" section only) |
| A platform or infra engineer deploying it | 00, 01, 04, 06, 07 or 08, 09 | 02, 03, 05 |
...
| Path | Purpose |
| --- | --- |
| `sample/my-agent/` | A uv project with two graphs: `echo` (deterministic, no model key needed) and `agent` (a tool-calling agent built with `create_agent`). |
...
| `sample/docker-compose.yml` | Local pre-flight of the built image with PostgreSQL and Redis. |
...
LangChain documentation, since no self-hosted LangSmith control plane was available.
